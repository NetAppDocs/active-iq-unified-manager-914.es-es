...
A partir de Unified Manager 9,8, VMware Tools se ha sustituido por Open VM Tools  `_open-vm-tools_`). No tiene que instalar VMware Tools como parte del proceso de instalación porque `_open-vm-tools_` se incluye con el paquete de instalación de Unified Manager.
...
Puede acceder a la interfaz de usuario web de para realizar la configuración inicial de Unified Manager, tal y como se describe en la link:../config/concept_configure_unified_manager.html["Configurando Active IQ Unified Manager"].

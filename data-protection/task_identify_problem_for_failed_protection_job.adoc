...
+
El trabajo implicó una relación de protección entre el volumen de origen `cluster2_src_vol2` en el servidor virtual `cluster2_src_svm` y el volumen de destino `managed_svc2_vol3` en el servidor virtual denominado `cluster3_dst_svm`.

* Se produjo un error en un trabajo de copia Snapshot para `0426cluster2_src_vol2snap` el volumen de origen `cluster2_src_svm:/cluster2_src_vol2` .
...
+
Se muestra la página de detalles Volumen / Estado para `cluster2_src_vol2`, en la que se muestra el contenido de la pestaña Protección.
...
+
La información de capacidad de las pantallas del volumen `cluster2_src_vol2`.
...
. En el área *Resumen*, lees el campo Causa del evento: `The full threshold set at 90% is breached. 45.38 MB (95.54%) of 47.50 MB is used`.
...
. Vaya a System Manager o acceda a la CLI de ONTAP para habilitar `volume autogrow` la opción.
+
[TIP]
====

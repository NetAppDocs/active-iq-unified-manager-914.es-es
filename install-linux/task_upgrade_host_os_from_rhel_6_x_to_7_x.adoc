...
Vea link:../health-checker/concept_manage_backup_and_restore_operations.html["Gestión de operaciones de backup y restauración"] si necesita revisar instrucciones detalladas para los procesos de copia de seguridad y restauración.
...
. En el sistema Red Hat Enterprise Linux 7.x, en el menú Administration de la interfaz de usuario web de, cree un backup de Unified Manager y, a continuación, copie el archivo de backup(`.7z`) y el contenido del subdirectorio del repositorio de base de datos (`/database-dumps-repo`en una ubicación externa.
. En el sistema Red Hat Enterprise Linux 7.x, apague Unified Manager.
. En el sistema Red Hat Enterprise Linux 8.x, copie el archivo de copia de seguridad(`.7z`) de la ubicación externa en `/opt/netapp/data/ocum-backup/` y los archivos de repositorio de base de datos en el `/database-dumps-repo` subdirectorio del `/ocum-backup` directorio.
...
. En el menú Administration de la interfaz de usuario web, cree un backup de Unified Manager y, a continuación, copie el archivo de backup(`.7z`) y el contenido del subdirectorio del repositorio de base de datos (`/database-dumps-repo`en una ubicación externa.
...
. Copie el archivo de copia de seguridad(`.7z`) de la ubicación externa en `/opt/netapp/data/ocum-backup/` y los archivos del repositorio de la base de datos en el `/database-dumps-repo` subdirectorio del `/ocum-backup` directorio.
. Introduzca el siguiente comando para restaurar la base de datos de Unified Manager desde el archivo de backup:
+
`um backup restore -f /opt/netapp/data/ocum-backup/<backup_file_name>`

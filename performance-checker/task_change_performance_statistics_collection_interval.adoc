...
El problema de las recopilaciones de estadísticas de rendimiento que no terminan a tiempo se indica mediante los mensajes de banner `Unable to consistently collect from cluster <cluster_name>` o `Data collection is taking too long on cluster <cluster_name>`.

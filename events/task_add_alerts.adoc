...
+
.. Introduzca *`abc`* en el campo *Name contains* para mostrar los volúmenes cuyo nombre contiene “abc”.
.. Seleccione *+<<All Volumes whose name contains 'abc'>>+* en el área Recursos disponibles y muévalo al área Recursos seleccionados.
.. Haga clic en *Excluir* e introduzca *`xyz`* en el campo *El nombre contiene* y, a continuación, haga clic en *Agregar*.

...
. Cree una nueva hoja (image:../media/excel_new_sheet_icon.png[""]) después de la `data` hoja y asígnele el nombre *Total Data Capacity*.
...
. Haga clic con el botón derecho en el gráfico y seleccione *Mover gráfico* para mover el gráfico a la `Total Data Capacity` hoja.

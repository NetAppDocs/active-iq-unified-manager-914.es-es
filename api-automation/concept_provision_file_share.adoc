...
[NOTE]
====
Puede recuperar los detalles de los niveles de servicio de rendimiento definidos por el sistema definiendo `system_defined` el parámetro de entrada en `true`. En el resultado, obtenga la clave del nivel de servicio de rendimiento que desea aplicar en el recurso compartido de archivos.
...
+
Puede ver que el método POST de `/storage-provider/file-shares` invoca internamente todas las API necesarias para cada una de las funciones y crea el objeto. Por ejemplo, invoca a `/storage-provider/performance-service-levels/` la API para asignar el nivel de servicio de rendimiento en el recurso compartido de archivos.

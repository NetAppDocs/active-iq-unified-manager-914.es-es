...
+
Ejemplo: Suponga que el clúster A tiene el almacén de datos A. Se exporta el almacén de datos A a través de LIF de datos con la misma dirección IP x.x.x.x y se crea una máquina virtual A en este almacén de datos. De igual modo, el clúster B tiene almacén de datos B. El almacén de datos B se exporta a través de LIF de datos con la misma dirección IP x.x.x.x y VM B se crea en el almacén de datos B. UM no podrá asignar el almacén de datos A para la topología de la máquina virtual A al correspondiente volumen/LUN de ONTAP ni asignar la máquina virtual B.
